...
class UsaTreasuryExchangeRateApi implements ExchangeRateApi {
    ...
    private static final Log LOG = LogFactory.getLog(UsaTreasuryExchangeRateApi.class);
    ...
    @Autowired
    public UsaTreasuryExchangeRateApi(WebClient.Builder webClientBuilder) {
    ...
    public UsaTreasuryExchangeRateApi(WebClient webClient, Duration apiTimeout, int apiRequestMaxAttempts, Duration apiRequestBackoffDuration) {
    ...
    @Override
    @Cacheable(
            value = UsaTreasuryExchangeRateCacheConfig.EXCHANGE_RATE_CACHE_NAME,
            key = "{#currency, #date}",
            cacheManager = "registerUsaTreasuryExchangeRateCacheManager"
    )
    @CircuitBreaker(
            name = UsaTreasuryExchangeRateCircuitBreakConfig.FIND_EXCHANGE_RATE_CIRCUIT_BREAKER_NAME,
    ...
    public Optional<BigDecimal> findByCurrencyAndDate(String currency, LocalDate date) {
        ...
        UsaTreasuryExchangeRateDto exchangeRate = exchangeRates.stream()
                .max(Comparator.comparing(UsaTreasuryExchangeRateDto::date))
                .orElseThrow();
    ...
    List<UsaTreasuryExchangeRateDto> findByCurrencyAndStartDateAndEndDate(String currency, LocalDate startDate, LocalDate endDate) {
        return this.webClient.get()
                ...
                .map(response -> response.data().stream()
                        .map(usaTreasuryExchangeRateDto -> usaTreasuryExchangeRateDto)
                        .toList()
                )
